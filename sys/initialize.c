...
/* Virtual Heap start and end */
#define HEAPSTART	(4096 * 4 * 1024)
#define HEAPEND		(4 * 1024 * 1024 * 1024)
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	setpdbr(NULLPROC);
	//set_evec(14,pfintr);
	kprintf("successfully installed pfint");
	enable_paging();

        fifo_head.fr_id = UNDEFINED;
        fifo_head.fr_next = NULL;
	kprintf("head : %x\n",fifo_head);
	kprintf("next: %x\n",fifo_head.fr_next);
