					...
					/* array in the code)		*/
{
	...
	//kprintf("starting pointer for normal process\n");
	numproc++;
	pptr = &proctab[pid];
...
/* emulate 386 "pushal" instruction */
	...
	*pushsp = pptr->pesp = (unsigned long)saddr;
	//DEMAND PAGING
	//kprintf("demand paging here\n");
	for(i = 0; i < NBS; i++)
	{
		pptr -> loc_bsm[i].bs_status = BSM_UNMAPPED;
                pptr -> loc_bsm[i].bs_pid = UNDEFINED;
                pptr -> loc_bsm[i].bs_vpno = UNDEFINED;
                pptr -> loc_bsm[i].bs_npages = 0;
                pptr -> loc_bsm[i].bs_sem = UNDEFINED;
                pptr -> loc_bsm[i].bs_private = SHARED;
	}
	//kprintf("initialised local BS\n");
	pptr->store = UNDEFINED;
	pptr->vhpno = UNDEFINED;
	pptr->vhpnpages = UNDEFINED;
	pptr->vmemlist = (struct mblock*)NULL;
	//kprintf("creating a PD\n");
	int status = pdcreate(pid);
	if(status == SYSERR){
		restore(ps);
		return SYSERR;
	}
	//kprintf("successfully created \n");
	restore(ps);
	return(pid);
}
